 ...
 * Table helper
 *
 * @uses viewHelper Zend_View_Helper_Html
 */
class Zend_View_Helper_HtmlTable {
    ...
    public $_noDataText = "No data found";
    ...
    private function _renderColumnText($name, $head)
    {
        ...
        $xhtml = '<a class=\'sortable\' href=\'' . $this->view->url(array(
        ...
        if (null !== $this->_data && $this->_data !== false && count($this->_data) > 0) {
            ...
            foreach ($this->_data as $key => $row) {
                ...
                foreach ($this->_columns as $key => $column) {
                	//Zend_Debug::dump($column,$key,true);
                    $td_class="";
                    if(array_key_exists('td_class', $column))
                    	$td_class = $column['td_class'];
                    $this->_output .= "<td class='$td_class' >";
                    //echo "<br> _getData($row, $key, $column); ";
    ...
    private function _getData($row, $key, $column)
    {
        ...
        } elseif (is_array($row) && array_key_exists($key, $row)) {
        	/*echo Zend_Debug::dump($row,'Row',false);
            echo Zend_Debug::dump($key,'key',false);
            echo Zend_Debug::dump($column,'Col',false);*/
            $output = $row[$key];
        } else {
            $output = '';
        }
        ...
        if (null !== $helper) {
            ...
            if ($helper instanceof Zend_View_Helper_Html_Table_RowInterface) {
    ...
    {
        if (!is_null($this->_noDataText)) {
        	if(!isset($this->_countColumns))
        		$this->_countColumns = 1;
